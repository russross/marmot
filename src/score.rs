    ...
    fn debug(&self, input: &Input) -> String {
        ...
        for elt in &self.sections_with_penalties {
            write!(
                &mut s,
                " {}:{}",
                input.sections[elt.section].get_name(),
                elt.penalty
            )
            .unwrap();
        }
...
impl ScoreCriterion for AntiConflictCriterion {
    ...
    fn debug(&self, input: &Input) -> String {
        ...
        write!(
            ...
            self.penalty,
            input.sections[self.single].get_name()
        )
        .unwrap();
        let mut sep = "";
        for &other in &self.group {
            write!(&mut s, "{}{}", sep, input.sections[other].get_name()).unwrap();
            sep = ", ";
        }
        s
    }
    ...
    fn debug(&self, input: &Input) -> String {
        ...
        for group in &self.grouped_by_days {
            ...
            for &sec in &self.sections {
                write!(&mut s, "{sep}{}", input.sections[sec].get_name()).unwrap();
                sep = ", ";
            }
...
impl ScoreCriterion for InstructorRoomCountCriterion {
    ...
    fn debug(&self, input: &Input) -> String {
        ...
        for &elt in &self.sections {
            write!(&mut s, "{}{}", sep, input.sections[elt].get_name()).unwrap();
            sep = ", ";
        }
        s
    }
