...
pub fn encode_criterion(
    ...
    match criterion {
        SatCriterion::Conflict { sections, priority } => encode_conflict(input, encoding, *priority, *sections),
        ...
        SatCriterion::FacultyClusterTooLong { faculty, days_to_check, duration, max_gap_within_cluster, priority } => {
            encode_faculty_cluster_too_long(
                input,
                encoding,
                *priority,
                *faculty,
        ...
        SatCriterion::FacultyClusterTooShort { faculty, days_to_check, duration, max_gap_within_cluster, priority } => {
            encode_faculty_cluster_too_short(
                input,
                encoding,
                *priority,
                *faculty,
...
fn encode_conflict(input: &Input, encoding: &mut Encoding, priority: u8, sections: [usize; 2]) -> Result<()> {
...
fn encode_anti_conflict(
    ...
    if !has_shared_time_slot {
        return err(format!(
            "Anti-conflict for section {} has no shared time slots with any group section",
...
fn encode_time_slot_preference(
    ...
    if !input.sections[section].time_slots.iter().any(|ts| ts.time_slot == time_slot) {
        return err(format!(
            "Time slot {} is not available for section {}",
...
fn encode_room_preference(
    ...
    if !input.sections[section].rooms.iter().any(|r| r.room == room) {
        return err(format!(
            "Room {} is not available for section {}",
...
fn make_section_pattern_vars(
    ...
    for &section in sections {
        ...
        for &TimeSlotWithOptionalPriority { time_slot, .. } in &input.sections[section].time_slots {
            ...
            let pattern = Pattern { days_count: time_slot_data.days.len(), duration: time_slot_data.duration };

            // Get or create a variable for this pattern
            if let Entry::Vacant(e) = pattern_to_var.entry(pattern) {
...
fn encode_time_pattern_match(input: &Input, encoding: &mut Encoding, priority: u8, sections: &[usize]) -> Result<()> {
    ...
    if pattern_to_var.len() <= 1 {
        return err(format!(
            "TimePatternMatch constraint trivial with fewer than 2 patterns for sections: {:?}",
            sections
        ));
    }
...
fn encode_faculty_days_off(
    ...
    if input.faculty[faculty].sections.len() <= 1 {
        return err(format!(
            "Faculty {} must have multiple sections with a days off constraint",
...
pub fn make_faculty_time_slot_vars(
    ...
    for &section in &input.faculty[faculty].sections {
        ...
        for &TimeSlotWithOptionalPriority { time_slot, .. } in &input.sections[section].time_slots {
            ...
            if let Entry::Vacant(e) = time_slot_to_var.entry(time_slot) {
                let var = encoding.new_var();
                e.insert(var);
                var_to_section_time_vars.insert(var, Vec::new());
            }
...
fn get_time_slot_clusters(
    ...
    for (current_start, current_end) in day_intervals.iter().skip(1) {
        // Calculate gap
...
// Represents a possible schedule pattern for a faculty member on a specific day
pub struct FacultyDayPattern {
    // The day of the week (0-6)
    pub day: u8,

    // The time slots for this pattern, each with a flag indicating if it's used
    pub time_slots: Vec<(usize, bool)>,

    // The resulting teaching clusters (start_time, end_time)
    pub clusters: Vec<(Time, Time)>,
}
...
fn get_faculty_day_patterns(
    ...
    if input.faculty[faculty].sections.len() <= 1 {
        return err(format!(
            "Faculty {} must have multiple sections for cluster constraints",
            input.faculty[faculty].name
        ));
    }
    ...
    for &section in &input.faculty[faculty].sections {
        let time_slots: Vec<usize> = input.sections[section]
            .time_slots
            .iter()
            .map(|ts| ts.time_slot)
    ...
    for day in days_to_check.into_iter() {
        ...
        let day_time_slots: Vec<usize> =
            potential_time_slots.iter().filter(|&&ts| input.time_slots[ts].days.contains(day)).cloned().collect();
        ...
        fn generate_patterns(
            ...
            day: u8,
            max_gap: Duration,
            result: &mut Vec<FacultyDayPattern>,
        ) -> Result<()> {
            if index >= pattern.len() {
                // We have a complete pattern
                // Extract the used time slots
                let used_time_slots: Vec<usize> =
                    pattern.iter().filter_map(|&(ts, used)| if used { Some(ts) } else { None }).collect();
                ...
                // Add to result
                result.push(FacultyDayPattern { day, time_slots: pattern.clone(), clusters });
                return Ok(());
            }

            // Try with this time slot unused
            pattern[index].1 = false;
            generate_patterns(input, pattern, index + 1, day, max_gap, result)?;
        ...
        generate_patterns(input, &mut pattern, 0, day, max_gap_within_cluster, &mut result)?;
    }
...
pub fn encode_faculty_cluster_helper(
    ...
    let day_patterns =
        get_faculty_day_patterns(input, faculty, days_to_check, max_gap_within_cluster, &faculty_time_slot_vars)?;

    // Process each day pattern
    for FacultyDayPattern { day, time_slots: pattern, clusters } in day_patterns {
    ...
    let count_too_long_gaps = move |clusters: &[(Time, Time)], _day: u8| -> usize {
        ...
        for i in 1..clusters.len() {
            ...
            if gap_duration > max_gap_duration {
                ...
                let already_caught = higher_priority_durations.iter().any(|&higher_dur| gap_duration > higher_dur);
...
fn get_faculty_time_slot_combos(
    ...
    for &section in faculty_sections {
        ...
        for &TimeSlotWithOptionalPriority { time_slot, .. } in &section_obj.time_slots {
            ...
            let meets_on_day_to_check = days_to_check.into_iter().any(|day| time_slot_obj.days.contains(day));
    ...
    for &section in &input.faculty[faculty].sections {
        ...
        for day in days_to_check.into_iter() {
            ...
            for &TimeSlotWithOptionalPriority { time_slot, .. } in &input.sections[section].time_slots {
                ...
                if let Entry::Vacant(e) = section_day_to_var.entry((section, day)) {
                    let var = encoding.new_var();
                    e.insert(var);
...
pub fn encode_faculty_evenly_spread(
    ...
    if days_to_check.len() <= 1 {
        return err(format!(
            "Need at least two days to spread out classes for faculty {}",
            input.faculty[faculty].name
        ));
    }
    if input.faculty[faculty].sections.len() <= 3 {
        return err(format!(
            "Faculty {} must have >3 sections to use evenly spread constraint",
            input.faculty[faculty].name
        ));
    }
    ...
    for &section in &input.faculty[faculty].sections {
        ...
        for &RoomWithOptionalPriority { room, .. } in &input.sections[section].rooms {
            ...
            for &TimeSlotWithOptionalPriority { time_slot, .. } in &input.sections[section].time_slots {
                ...
                if let Entry::Vacant(e) = room_time_to_var.entry((room, time_slot)) {
                    let var = encoding.new_var();
                    e.insert(var);
                ...
                var_to_section_room_time_pairs
                    .get_mut(&room_time_to_var[&(room, time_slot)])
                    .unwrap()
...
pub struct BackToBackSectionPair {
    // The day of the week (0-6)
    pub day: u8,

    // The section pair
    pub sections: [usize; 2],

    // Rooms they have in common
    pub common_rooms: Vec<usize>,
}
...
fn get_back_to_back_section_pairs(
    ...
    days_to_check: Days,
    max_gap: Duration,
) -> Result<Vec<BackToBackSectionPair>> {
    // Validate inputs
    if faculty >= input.faculty.len() {
        return err(format!("Faculty index {} not found in input", faculty));
    }
    ...
    for (i, &section_a) in faculty_sections.iter().enumerate() {
        for &section_b in faculty_sections.iter().skip(i + 1) {
            ...
            for day in days_to_check.into_iter() {
                ...
                let a_day_slots: Vec<_> = input.sections[section_a]
                    .time_slots
                    .iter()
                    .filter(|ts| input.time_slots[ts.time_slot].days.contains(day))
                    .collect();

                let b_day_slots: Vec<_> = input.sections[section_b]
                    .time_slots
                    .iter()
                    .filter(|ts| input.time_slots[ts.time_slot].days.contains(day))
                    .collect();
                    ...
                    for ts_b in &b_day_slots {
                        ...
                        if a_end.minutes <= b_start.minutes {
                            ...
                            if gap.minutes <= max_gap.minutes {
                                result.push(BackToBackSectionPair {
                                    day,
                                    sections: [section_a, section_b],
                                    common_rooms: common_rooms.clone(),
                                });
                                found_pair = true;
                                break;
                            }
                ...
                if !found_pair {
                    for ts_b in &b_day_slots {
                        let b_end =
                            input.time_slots[ts_b.time_slot].start_time + input.time_slots[ts_b.time_slot].duration;
...
fn is_back_to_back(input: &Input, ts1: usize, ts2: usize, max_gap: Duration) -> bool {
...
pub fn encode_faculty_no_room_switch(
    ...
    if input.faculty[faculty].sections.len() <= 1 {
        return err(format!(
            "Faculty {} should not have no room switch criterion with < 2 sections",
            input.faculty[faculty].name
        ));
    }
    ...
    if faculty_room_time_vars.is_empty() {
        return err(format!(
            "{} no room switch constraint found no rooms/time slots to consider",
            input.faculty[faculty].name
        ));
    }

    // Get all back-to-back section pairs with their common valid rooms
    let back_to_back_section_pairs =
        get_back_to_back_section_pairs(input, faculty, days_to_check, max_gap_within_cluster)?;
    ...
    for BackToBackSectionPair { day, sections, common_rooms } in back_to_back_section_pairs {
        ...
        let section1_time_slots: Vec<_> = input.sections[section1]
            .time_slots
            .iter()
            .filter(|ts| input.time_slots[ts.time_slot].days.contains(day))
            .map(|ts| ts.time_slot)
        ...
        let section2_time_slots: Vec<_> = input.sections[section2]
            .time_slots
            .iter()
            .filter(|ts| input.time_slots[ts.time_slot].days.contains(day))
            .map(|ts| ts.time_slot)
