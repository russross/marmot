...
pub fn generate_schedule(input: &Input) -> Result<Schedule> {
    ...
    println!("Loaded {} constraints across {} priority levels", 
    ...
    for priority in 0..=max_priority {
        ...
        println!("Solving for priority level {} with {} criteria", priority, constraints.len());
        ...
        match best {
            Some(_) => println!("  Successfully found solution with {} violations", max_violations.levels[priority as usize]),
            None => {
                ...
                if priority == 0 {
                    ...
                    return err("Failed to find a solution that satisfies all hard constraints");
                }
                break;
            }
        }
    ...
    if let Some(schedule) = best {
        ...
        println!("\nFinal solution score: {}", schedule.score);
        println!("Total solving time: {:?}", start_time.elapsed());
...
fn solve_at_priority_level(
    ...
    while max_violations.levels[priority as usize] <= criteria_count as i16 {
        ...
        println!("    priority {}, violations={} solving encoding with {} variables and {} clauses", 
            priority, max_violations.levels[priority as usize], encoding.last_var, encoding.clauses.len());
        ...
        match encoding.solve() {
            ...
            Ok(None) => {
                ...
                if priority == 0 && max_violations.levels[priority as usize] == 1 {
                    println!("    could not find a solution using only hard constraints");
                }
            },
            Err(e) => {
...
fn create_sat_instance(
    ...
    for p in 0..=current_priority {
        ...
        let violations = if p < current_priority {
            max_violations.levels[p as usize]
        } else {
            max_violations.levels[p as usize]
        };
...
fn encode_room_conflicts(
    ...
    for (&(section, room), _) in &encoding.section_room_vars {
...
fn encode_room_conflict(
    ...
    let section_a_time_slots: Vec<usize> = input.sections[section_a].time_slots.iter()
        .map(|ts| ts.time_slot)
        .collect();
    let section_b_time_slots: Vec<usize> = input.sections[section_b].time_slots.iter()
        .map(|ts| ts.time_slot)
        .collect();
...
fn decode_solution(
    ...
    let var_to_section_room: HashMap<i32, (usize, usize)> = encoding.section_room_vars.iter()
        .map(|(&k, &v)| (v, k))
        .collect();
    let var_to_section_time: HashMap<i32, (usize, usize)> = encoding.section_time_vars.iter()
        .map(|(&k, &v)| (v, k))
        .collect();
    ...
    // Use move_section to place each section in the schedule
    for (section, time_slot) in section_to_time_slot {
        let room = section_to_room.get(&section).copied();
        
        // Use move_section to update the schedule
    ...
    if found_score != expected_score {
        println!("\nWarning: Schedule score doesn't match expected score from problems up to priority {}", priority);
