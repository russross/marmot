...
fn main() -> Result<(), Box<rhai::EvalAltResult>> {
    ...
    for _ in 0..10000 {
        ...
        let &RoomTimeWithPenalty{room, time_slot, ..} = &section.room_times[rng.gen_range(0..section.room_times.len())];
        solver.remove_placement(section_i);
        solver.displace_conflicts(&term, section_i, room, time_slot);
        solver.add_placement(section_i, room, time_slot);
    }
...
impl Solver {
    ...
    fn add_placement(&mut self, section: usize, room: usize, time_slot: usize) {
    ...
    fn displace_conflicts(&mut self, input: &Input, section: usize, room: usize, time_slot: usize) {
    ...
    fn select_section_to_place(&mut self, input: &Input) {
        let mut rng = rand::thread_rng();
        let selection = rng.gen_range(0..input.sections.len());
    }

    fn print_schedule(&self, input: &Input) {
        let mut name_len = 0;
        for (instructor_i, instructor) in input.instructors.iter().enumerate() {
            for &section_i in &instructor.sections {
                ...
                if section.cross_listings.len() > 1 {
                    name_len = std::cmp::max(name_len, section.course.len() + section.section.len() + 2);
                } else {
                    name_len = std::cmp::max(name_len, section.course.len() + section.section.len() + 1);
                }
        ...
        for (time_slot_i, time_slot) in input.time_slots.iter().enumerate() {
            ...
            for room_i in 0..input.rooms.len() {
                if let Some(section_i) = self.room_placements[room_i].time_slot_placements[time_slot_i] {
