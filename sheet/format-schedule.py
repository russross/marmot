...
VALID_SUBJECTS = {'CS', 'IT', 'SD', 'SE', 'SET', 'UXD', 'ZZ'}
...
type BlockSessions = tuple[str, str]
type RawSection = tuple[str | None, str, str | None, str | None, str, str, float]

YELLOW = '#FFFF50'
GREEN = '#63BB36'
RED = '#AB3012'
NUMERIC_COLUMNS = frozenset({CREDITS, WLF, CAP, ENROLL, WAITCAP, WAITACT})
...
class OutputRow:
    values: list[str]
    state: RowState = RowState.NORMAL
    changed_columns: frozenset[int] = frozenset()

def validate_headings(actual: list[str]) -> None:
    mismatches = []
    column_count = max(len(actual), len(EXPECTED_COLUMNS))
    for index in range(column_count):
        expected = EXPECTED_COLUMNS[index] if index < len(EXPECTED_COLUMNS) else None
        found = actual[index] if index < len(actual) else None
        if expected != found:
            mismatches.append(
                f'column {index + 1}: expected {expected!r}, found {found!r}')

    if mismatches:
        details = '\n'.join(f'  {mismatch}' for mismatch in mismatches)
        raise ValueError(f'previous schedule headings do not match:\n{details}')

def get_old(filename: str) -> list[list[str]]:
    with open(filename, newline='') as fp:
        reader = csv.reader(fp)
        headings = next(reader, None)
        if headings is None:
            print('Error: previous schedule is empty', file=sys.stderr)
            sys.exit(1)
        try:
...
def get_block_sessions(term: str) -> BlockSessions:
    semester = term.partition(' ')[0].casefold()
    if semester == 'spring':
        return ('2', '3')
    if semester == 'fall':
        return ('6', '7')
    raise ValueError(f'unsupported semester in database term {term!r}')

def get_new(filename: str, id: int) -> tuple[list[RawSection], BlockSessions]:
    ...
    term_row = db.execute('SELECT term FROM terms').fetchone()
    if term_row is None:
        print('Error: Database has no term')
        sys.exit(1)
    try:
        block_sessions = get_block_sessions(term_row[0])
    except ValueError as error:
        print(f'Error: {error}')
        sys.exit(1)

    course_titles = dict(db.execute('SELECT course, course_name FROM courses'))
    section_credits = dict(db.execute('SELECT section, credit_hours FROM sections'))
    cross_listings: dict[str, list[str]] = {}
    for secondary, primary in db.execute('''
            SELECT section, primary_section
            FROM cross_listing_sections
            ORDER BY primary_section, section'''):
        cross_listings.setdefault(primary, []).append(secondary)
    ...
    for (section, room, time_slot, instructors_str) in rows:
        ...
        course_key = section[:section.index('-')]
        title = course_titles.get(course_key, '')
        secondary_sections = cross_listings.get(section, [])
        primary_annotations = [xlist] if xlist else []
        primary_annotations.extend(f'xlist with {secondary}' for secondary in secondary_sections)
        lst.append((primary, section, room, time_slot, '; '.join(primary_annotations), title,
                    section_credits[section]))

        for secondary in secondary_sections:
            secondary_course = secondary[:secondary.index('-')]
            annotations = [annotation for annotation in (xlist, f'xlist with {section}')
                           if annotation]
            lst.append((primary, secondary, room, time_slot, '; '.join(annotations),
                        course_titles.get(secondary_course, ''), section_credits[secondary]))
    print(f'read placement {id} with {len(lst)} placements')
    db.close()
    return (lst, block_sessions)

def split_fields(row: RawSection, block_sessions: BlockSessions) -> list[str] | None:
    (raw_instructor, raw_course, raw_room, raw_time, xlist_annotation, course_title,
     credit_hours) = row
    ...
    if raw_room is not None and raw_time is None:
        ...
        # lecture class
        if raw_room is None:
            raise RuntimeError(f'section {raw_course} unexpectedly has no room')
        if raw_room != '':
    ...
    if section_n.endswith('A'):
        out[SESSION] = block_sessions[0]
    elif section_n.endswith('B'):
        out[SESSION] = block_sessions[1]
    else:
        out[SESSION] = '1'
...
def merge(past: list[list[str]], future: list[list[str]]) -> list[OutputRow]:
    out = [OutputRow(past[0])]
    past = past[1:]
    end_marker = future[-1][:]
    ...
    for (i, frow) in enumerate(future):
        ...
        while (prow[SUBJECT], prow[COURSE]) < this_course:
            if prow[CRN] == '' or skip_row(prow[SUBJECT], prow[COURSE], prow[SECTION]):
                out.append(OutputRow(prow, RowState.SKIP))
            else:
                out.append(OutputRow(prow, RowState.DELETE))
            past = past[1:]
            prow = past[0]
        ...
        if (prow[SUBJECT], prow[COURSE]) == this_course:
            if sections_match(prow[SECTION], frow[SECTION]):
                ...
                out.append(OutputRow(frow, changed_columns=diff(prow, frow)))
                past = past[1:]
                matched = True
            else:
                ...
                if len(old_sections) == 1 and new_sections_remaining == 1:
                    ...
                    out.append(OutputRow(frow, changed_columns=diff(match_prow, frow)))
                    past = past[1:]
                    matched = True
        ...
        if not matched:
            ...
            out.append(OutputRow(frow, RowState.INSERT))
...
def diff(left: list[str], right: list[str]) -> frozenset[int]:
    changed_columns = set()
...
def cell_format(row: OutputRow, column: int, formats: dict[str, Format]) -> Format | None:
    if row.state == RowState.INSERT:
        return formats['insert']
    if row.state == RowState.DELETE:
        return formats['delete']
    if column in row.changed_columns:
        return formats['changed']
    return None
...
def write_cell(
    ...
    if row > 0 and column in NUMERIC_COLUMNS and value != '':
        sheet.write_number(row, column, float(value), output_format)
    else:
        sheet.write(row, column, value, output_format)

def write_xlsx(filename: str, rows: list[OutputRow]) -> None:
    with Workbook(filename) as workbook:
        sheet = workbook.add_worksheet('Schedule')
        formats = {
            'changed': workbook.add_format({'bg_color': YELLOW}),
            'insert': workbook.add_format({'bg_color': GREEN}),
            'delete': workbook.add_format({'bg_color': RED}),
        }

        for row_index, row in enumerate(rows):
            for column, value in enumerate(row.values):
                write_cell(sheet, row_index, column, value,
                           cell_format(row, column, formats))

        for column in range(XLIST + 1):
            width = max(len(row.values[column]) for row in rows) + 2
            sheet.set_column(column, column, min(width, 50))

def main():
    old = get_old(PREVIOUS_FILE)
    raw_sections, block_sessions = get_new(DB_FILE, ID)
    new = [split_fields(row, block_sessions) for row in raw_sections]
    new = [row for row in new if row is not None]
    end_marker = split_fields(
        ('', 'ZZ 1111-11', 'Smith 109', 'MWF0700+50', '', '', 3.0),
        block_sessions)
    if end_marker is None:
        raise RuntimeError('could not create the schedule end marker')
    new.append(end_marker)
    new.sort(key=lambda row: (row[SUBJECT], row[COURSE], row[SECTION], row[DAYS] if len(row[DAYS]) > 1 else '~'+row[DAYS], row[BEGTIME]))
    out = merge(old, new)
    write_xlsx(NEW_XLSX, out)
